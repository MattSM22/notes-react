...
export function NewNoteCard({ onNoteCreated }: newNoteCardProps) {
  ...
  function handleStartRecording(){
    const isSpeechReecognitionAPIAvailable = 'SpeechRecognition' in window
      || 'webkitSpeechRecognition' in window;

    if (!isSpeechReecognitionAPIAvailable) {
  ...
  return (
    <Dialog.Root>
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="inset-8 fixed bg-black/60" />
        <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60dvh] bg-slate-700 md:rounded-md flex flex-col outline-none">
